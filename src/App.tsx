...
export default function App() {
  ...
  useEffect(() => {
    const syncWithCloud = async () => {
      try {
        ...
        if (response.ok) {
          ...
          let needsRemoteInit = false;

          // Check if server database is empty (no shops, no inventory, no orders)
          const isServerEmpty = !cloudData.shops && !cloudData.inventory && !cloudData.orders;

          if (!isServerEmpty) {
            // Update client state from server if server has actual records
            if (cloudData.shops && JSON.stringify(cloudData.shops) !== JSON.stringify(shopsRef.current)) {
              skipNextCloudPush.current.shops = true;
              setShops(cloudData.shops);
            }
            if (cloudData.inventory && JSON.stringify(cloudData.inventory) !== JSON.stringify(inventoryRef.current)) {
              skipNextCloudPush.current.inventory = true;
              setInventory(cloudData.inventory);
            }
            if (cloudData.orders && JSON.stringify(cloudData.orders) !== JSON.stringify(ordersRef.current)) {
              skipNextCloudPush.current.orders = true;
              setOrders(cloudData.orders);
            }
          } else {
            // Server has no records at all, we can seed it with local client data
            needsRemoteInit = true;
          }

          // Mark initial sync complete so future local changes are pushed
          isInitialSyncDone.current = true;

          if (needsRemoteInit && (shopsRef.current.length > 0 || inventoryRef.current.length > 0 || ordersRef.current.length > 0)) {
            ...
            console.log("[Sync] Initialized cloud database with client local storage state.");
          }
        }
      } catch (err) {
        console.error("Failed to sync with cloud on mount:", err);
        // Ensure user can still work locally
        isInitialSyncDone.current = true;
      }
    };
    ...
    // Poll server every 3 seconds for fast, snappy, real-time multi-device catalog syncing
    const intervalId = setInterval(async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Only update state and skip pushing back if server data is different from current ref
          if (cloudData.shops && JSON.stringify(cloudData.shops) !== JSON.stringify(shopsRef.current)) {
            skipNextCloudPush.current.shops = true;
            setShops(cloudData.shops);
          }
        ...
        console.error("Error during periodic cloud polling:", err);
      }
    }, 3000);
