...
export default function App() {
  ...
  useEffect(() => {
    const syncWithCloud = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // 1. Sync shops
          if (cloudData.shops) {
            if (JSON.stringify(cloudData.shops) !== JSON.stringify(shops)) {
          ...
          if (cloudData.inventory) {
            if (JSON.stringify(cloudData.inventory) !== JSON.stringify(inventory)) {
          ...
          // 3. Sync orders
          if (cloudData.orders) {
            if (JSON.stringify(cloudData.orders) !== JSON.stringify(orders)) {
          ...
          if (needsRemoteInit) {
            // Push initial client data to the server
    ...
    // Poll server every 8 seconds for multi-device catalog syncing
    const intervalId = setInterval(async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Safely check and only update state if server has newer/different data
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      triggerToast(`Error: ${err.message || "Could not send OTP"}`);
    } finally {
