...
export default function App() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (!isServerEmpty) {
            ...
            if (cloudData.shops && JSON.stringify(cloudData.shops) !== JSON.stringify(shops)) {
              skipNextCloudPush.current.shops = true;
              setShops(cloudData.shops);
            }
            if (cloudData.inventory && JSON.stringify(cloudData.inventory) !== JSON.stringify(inventory)) {
              skipNextCloudPush.current.inventory = true;
              setInventory(cloudData.inventory);
            }
            if (cloudData.orders && JSON.stringify(cloudData.orders) !== JSON.stringify(orders)) {
          ...
          if (needsRemoteInit && (shops.length > 0 || inventory.length > 0 || orders.length > 0)) {
    ...
    // Poll server every 6 seconds for multi-device catalog syncing
    const intervalId = setInterval(async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Only update state and skip pushing back if server data is different
          if (cloudData.shops && JSON.stringify(cloudData.shops) !== JSON.stringify(shops)) {
            skipNextCloudPush.current.shops = true;
            setShops(cloudData.shops);
          }
